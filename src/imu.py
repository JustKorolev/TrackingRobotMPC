...
class IMUGUI:
    def __init__(self, root, shared_state, sampling_rate=50, mpc_horizon=1, workspace_offset=np.eye(4,4)):
        ...
        # Change these when you know the exact mounting offset.
        # Identity (zeros) = robot base frame == IMU world frame.
        self._imu_offset_translation = np.array([0.0, 0.0, 0.0])
        self._imu_offset_roll  = 0.0
        self._imu_offset_pitch = 0.0
        self._imu_offset_yaw   = 0.0
        self._T_robot_to_imu = self._build_robot_to_imu_transform()
    ...
    def _build_robot_to_imu_transform(self):
        R = self.rotation_matrix(
            self._imu_offset_roll,
            self._imu_offset_pitch,
            self._imu_offset_yaw)
        T = np.eye(4)
        T[:3, :3] = R
        T[:3, 3] = self._imu_offset_translation
        return T

    def robot_base_to_imu_world(self, positions_robot, orientations_robot=None):
        """p_imu = R_offset @ p_robot + t_offset"""
        R = self._T_robot_to_imu[:3, :3]
        t = self._T_robot_to_imu[:3, 3]
        positions_imu = (R @ positions_robot.T).T + t
        if orientations_robot is not None:
            orientations_imu = orientations_robot.copy()
            for i in range(len(orientations_robot)):
                R_rob = self.rotation_matrix(*orientations_robot[i])
                R_imu = R @ R_rob
                orientations_imu[i, 0] = np.arctan2(R_imu[2, 1], R_imu[2, 2])
                orientations_imu[i, 1] = np.arcsin(np.clip(-R_imu[2, 0], -1, 1))
                orientations_imu[i, 2] = np.arctan2(R_imu[1, 0], R_imu[0, 0])
        else:
            orientations_imu = None
        return positions_imu, orientations_imu

    def imu_world_to_robot_base(self, positions_imu, orientations_imu=None):
        """p_robot = R_offset^T @ (p_imu - t_offset)"""
        R = self._T_robot_to_imu[:3, :3]
        t = self._T_robot_to_imu[:3, 3]
        R_inv = R.T
        positions_robot = (R_inv @ (positions_imu - t).T).T
        if orientations_imu is not None:
            orientations_robot = orientations_imu.copy()
            for i in range(len(orientations_imu)):
                R_imu = self.rotation_matrix(*orientations_imu[i])
                R_rob = R_inv @ R_imu
                orientations_robot[i, 0] = np.arctan2(R_rob[2, 1], R_rob[2, 2])
                orientations_robot[i, 1] = np.arcsin(np.clip(-R_rob[2, 0], -1, 1))
                orientations_robot[i, 2] = np.arctan2(R_rob[1, 0], R_rob[0, 0])
        else:
            orientations_robot = None
    ...
    def stream_trajectory_to_window(self):
        ...
        try:
            while self.running:
                ...
                if not following:
                    self.set_status("Trajectory streaming stopped")
                    break
                ...
                if prev_t is None:
                    ...
                    if self._has_pixhawk_attitude:
                        ...
                        a0n = np.linalg.norm(accel)
                        if a0n > 1e-6:
                            roll0 = np.arctan2(accel[1], accel[2])
                            pitch0 = np.arctan2(-accel[0], np.sqrt(accel[1]**2 + accel[2]**2))
                        else:
                            roll0, pitch0 = 0.0, 0.0
                        q = self.euler_to_quat(roll0, pitch0, 0.0)
                ...
                is_still = self.is_stationary_sample(accel, gyro)

                if is_still:
                    ba = self.GYRO_BIAS_ALPHA
                    gyro_bias_online = (1 - ba) * gyro_bias_online + ba * gyro

                omega = gyro - gyro_bias_online
                omega_q = np.array([0.0, omega[0], omega[1], omega[2]])
                q_dot = 0.5 * self.quat_multiply(q, omega_q)
                q = q + q_dot * dt
                q = q / np.linalg.norm(q)
                ...
                if is_still and not self._has_pixhawk_attitude:
                    ...
                    if accel_norm > 1e-6:
                        R_cur = self.quat_to_rotation_matrix(q)
                        g_meas = accel / accel_norm
                        g_expected = R_cur.T @ np.array([0.0, 0.0, 1.0])
                        correction = np.cross(g_meas, g_expected) * self.ACCEL_CORRECTION_GAIN
                        cq = np.array([0.0, correction[0], correction[1], correction[2]])
                        q_dot_c = 0.5 * self.quat_multiply(q, cq)
                        q = q + q_dot_c * dt
                        q = q / np.linalg.norm(q)
                ...
                gravity_body = R_enu_t.T @ gravity_enu
                a_lin_nav = R_nb @ (accel - gravity_body)

                orientation[0] = np.arctan2(R_nb[2, 1], R_nb[2, 2])
                sinp = np.clip(-R_nb[2, 0], -1.0, 1.0)
                orientation[1] = np.arcsin(sinp)
                orientation[2] = np.arctan2(R_nb[1, 0], R_nb[0, 0])
